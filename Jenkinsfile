        ...
        stage('Test') {
            steps {
                sh '''
                    ...
                    # Download JUnit jar only if not already present
                    ...
                    # Verify that the test files exist (you may need to adjust the test path here)
                    echo "Listing test directory contents:"
                    ls test

                    # Compile test files (make sure your tests are under the 'test' folder or adjust this path)
                    mkdir -p test-build
